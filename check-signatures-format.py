...
for file_name in sorted(os.listdir("_data/signed")):
    if not file_name.endswith(".yaml") or file_name[:-5] != file_name[:-5].strip():
        print(file_name, "probably has an invalid name. You may (1) wait for review (2) use a real name instead (3) add .yaml as file extension (4) remove extra whitespaces from the file name")
        ok = False
